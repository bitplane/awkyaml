...
function yaml_event_escape_path(text,    out, i, ch) {
    out = ""
    for (i = 1; i <= length(text); i++) {
        ch = substr(text, i, 1)
        if (ch == "\\") {
...
function yaml_event_emit_map_start(doc_id, path, tag, anchor) {
    print "MAP_START" "\t" doc_id "\t" yaml_event_escape_path(path) "\t" yaml_event_escape(tag) "\t" yaml_event_escape(anchor)
}

function yaml_event_emit_map_end(doc_id, path) {
    print "MAP_END" "\t" doc_id "\t" yaml_event_escape_path(path)
}

function yaml_event_emit_seq_start(doc_id, path, tag, anchor) {
    print "SEQ_START" "\t" doc_id "\t" yaml_event_escape_path(path) "\t" yaml_event_escape(tag) "\t" yaml_event_escape(anchor)
}
